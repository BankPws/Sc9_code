...
import com.example.sc9.SendNotification.APIService;
import com.example.sc9.SendNotification.Client;
import com.example.sc9.SendNotification.Data;
import com.example.sc9.SendNotification.MyResponse;
import com.example.sc9.SendNotification.NotificationSender;
...
public class ChefPreparedOrderView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Shipper = findViewById(R.id.shipp);
                                                ...
                                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                    ...
                                                    FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(deliveryId).child(RandomUID).child("OtherInformation").setValue(hashMap1).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        ...
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            FirebaseDatabase.getInstance().getReference("CustomerFinalOrders").child(userid).child(RandomUID).child("OtherInformation").child("Status").setValue("Order is Success...").addOnSuccessListener(new OnSuccessListener<Void>() {
                                                                @Override
                                                                ...
                                                                public void onSuccess(Void aVoid) {
                                                                    FirebaseDatabase.getInstance().getReference().child("Tokens").child(deliveryId).child("token").addListenerForSingleValueEvent(new ValueEventListener() {
                                                                        ...
                                                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                                            ...
                                                                            builder.setMessage("Order has been sent to customer");
